<?php
/*
Plugin Name: RO Product Picker
Plugin URI: http://reneorozco.me/proyectos/plugins/ro-product-picker/
Description: This plugin let you select multiple product combinations to add to cart.
Version: 3.0
Author: René Orozco
Author URI: http://reneorozco.me
License:
*/

require 'inc/plugin-update-checker.php';
$MyUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
    'http://reneorozco.me/proyectos/plugins/ro-product-picker/metadata.json',
    __FILE__,
    'ro-product-picker'
);
...
function pp_init(){
	...
	if(!is_dir($path) && wp_mkdir_p($path)) echo  ' --- Directorio creao en: '.$path.' --- ';
}
...
function ro_pp_scripts(){
	...
	wp_enqueue_script(	'ro_pp_script',				plugins_url( 'res/scripts.js', __FILE__ ), array('jquery'), '1.2', true );
}
add_action( 'wp_enqueue_scripts', 'ro_pp_scripts' );
add_action( 'admin_print_styles', 'ro_pp_scripts' );
function pp_ajaxurl() {
	global $post;
...
<div class="wrap">
	<form action="options.php" method="post">
		...
		<?php do_settings_sections('ro-product-picker'); ?>
		<?php submit_button(); ?>
	</form>
</div>
<?php
...
function pp_menu(){
	add_options_page('Product Picker', 'Product Picker', 'administrator', 'ro-product-picker', 'pp_options_page');
}
...
function pp_settings(){
	...
	function pp_power_str(){
		...
		echo "<input name=\"pp_options[power]\" type=\"radio\" value=\"on\"".($options['power'] == 'on' ? ' checked' : '')." /> Encendido <input name=\"pp_options[power]\" type=\"radio\" value=\"off\"".($options['power'] != 'on' ? ' checked' : '')." /> Apagado";
	}
	...
	add_settings_section('pp_main', 'Product Picker', 'pp_main_text', 'ro-product-picker');
	add_settings_field('pp_power', 'Estado: ', 'pp_power_str', 'ro-product-picker', 'pp_main');
	add_settings_field('pp_embolsado', 'Precio de embolsado: ', 'pp_embolsado_str', 'ro-product-picker', 'pp_main');
	add_settings_field('pp_tipos_estampacion', 'Tipos de estampación: ', 'pp_tipos_estampacion_str', 'ro-product-picker', 'pp_main');
	add_settings_field('pp_porcentajes', 'Reducción del precio de acuerdo a la cantidad de elementos: ', 'pp_porcentajes_str', 'ro-product-picker', 'pp_main');
}
...
function ro_pp_custom_meta_callback(){
	...
	<table>
		<tr><th colspan="3" style="text-transform: uppercase;text-align: center;">Tipos de estampación disponibles para el producto</th></tr>
<?php
	$options = get_option('pp_options');
	$tipos = array();
	$tipos_full = array();
...
<style>
.woo-product1 h1, .woo-product1 h2 {margin: 0;padding: 0;}
.woocommerce div.product form.cart .variations {display: none;}
.woocommerce div.product .range-price {border: darkgray 1px solid;display: inline-block;padding: 7px 0;vertical-align: top;}
	.woocommerce div.product .range-price > span {color: black;display: block;line-height: 19px;padding-right: 15px;text-align: right;width: 120px;}
.woocommerce-variation, .woocommerce-variation-add-to-cart {display: none !important;}
</style>
...
<?php
		...
		<div id="pp_sizes" class="col-xs-12 col-sm-8">
			<div class="pp_cbar">
				<span class="bar"><span></span></span>
				0
				<div class="pull-right"><span style="color: red">Cantidad mínima de pedido | </span><span id="pp_total">0</span>/<span id="pp_total50"><?php echo (int) $meta['minimo'] > 0 ? (int) $meta['minimo'] : 50; ?></span> UNDS.</div>
			</div>
<?php
	$colores = array();
	$_product = wc_get_product(get_queried_object_id());
			...
			foreach($variations2 as $k2 => $v2){
				if($v2->attributes['pa_color'] == $value['color'] && $v2->attributes['pa_tamano'] == $v) $vid = $v2->slug;
						...
						<span>
							...
							<input type="number" name="<?php echo $vid; ?>" value="0">
						</span>
<?php
		} ?>
					</div>
...
function get_prices() {
	...
	foreach($variations1 as $value){
		...
		if(isset($_POST[$single_variation->slug])){
			...
			if(array_search(strtolower('blanco'), array_map('strtolower', $attr)) !== false){
				$totalb += (int) $_POST[$single_variation->slug];
				foreach($tipos as $key => $value){
					if($get[0] == $value || $get[1] == $value){
					  $precioaux =  $price += (int) $_POST[$single_variation->slug];
					  $por=$pricb[$key];
						 $price += (int) $_POST[$single_variation->slug] * (float) $pricb[$key];
						if($addprice == 0) $addprice = (float) $pripp[$key];
					}
				}
			}else{
	...
	echo $total.'---'.$totalb.'---'.$totalc.'---'.strip_tags(html_entity_decode(wc_price($priceu))).'---'.wc_price($price).'---'.wc_price($price + ($price / 100 * $tax_p)).'---'.$price.'---'.$description.'---'.$precioaux.'---'.$por.'---'.$pripp[$key].'---'.$addprice ;
	wp_die();
}
